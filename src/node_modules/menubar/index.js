  ...
  function appReady () {
    ...
    if (process.platform === 'darwin') {
      ...
      if (osxPrefs.isDarkMode() === true) {
        var iconPath = opts.icon || path.join(opts.dir, 'assets/icon.png')
      } else { 
        var iconPath = opts.icon || path.join(opts.dir, 'assets/iconblack.png')
       }
    } else { var iconPath = opts.icon || path.join(opts.dir, 'assets/icon.png') }
    ...
    function createWindow () {
      ...
      if (process.platform === 'darwin') {
        const osxPrefs = require('electron-osx-appearance');
        if (osxPrefs.isDarkMode() === true) {
          console.log('os is running in dark mode')
          // vibrancy dark
          // icon white
          var vibrancy = { vibrancy: 'dark' }
          var darkmode = true;
        } else { 
          console.log('os is not running in dark mode')
          // vibrancy light
          // icon black
          var vibrancy = { vibrancy: 'light' }
          var darkmode = false;
         }
      } 
      ...
      var winOpts = extend(defaults, opts, vibrancy)
    ...
    function showWindow (trayPos) {
      ...
      var y = (opts.y !== undefined) ? opts.y : position.y - 5
